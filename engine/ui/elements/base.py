...
class UIElement:
    """Base class for UI elements and panels"""
    ...
    def contains_point(self, x, y):
        """Check if a point is within the element"""
        return (self.x <= x <= self.x + self.width and
                self.y <= y <= self.y + self.height)
    
    def handle_event(self, event):
        """Handle events for the element"""
        if not self.visible:
            return False
    ...
    def render(self, screen):
        """Render the UI element with panel styling if needed"""
        if not self.visible:
            return
