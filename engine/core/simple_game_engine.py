...
class SimpleGameEngine:
    ...
    def _start_game_from_menu(self, seed=None, is_new_world=False, use_custom_map=False):
        ...
        try:
            print(f"Starting game from menu with seed: {seed}, new world: {is_new_world}, custom map: {use_custom_map}")
            self.state_manager.set_state(self.state_manager.game_state_constants.RUNNING)
            
            # Switch to game music
            self.sound_manager.play_music("track_game", loops=-1, fade_in=1000)
            
            # Handle custom map vs regular world
            if use_custom_map:
                print("Preparing to load custom map...")
                # For custom maps, generate a seed for the world cache
                if seed is None:
                    import random
            ...
            # If we have a world map already, handle map generation/loading
            if hasattr(self, 'world_map') and self.world_map:
                if use_custom_map:
                    # Load the custom map instead of generating
                    print("Loading custom map into world...")
                    if hasattr(self, 'world_cache'):
                        success = self.world_cache.load_custom_map(self.world_map)
                        if not success:
                        ...
                        print("No world cache available for custom map loading")
                        self.world_map.generate_realistic_map(seed=self.map_seed)
                else:
                    # Regular procedural generation
                    print(f"Regenerating world map with seed: {self.map_seed}")
                    self.world_map.generate_realistic_map(seed=self.map_seed)
                
                # Only load cached entities if this is NOT a new world and NOT a custom map
                if not is_new_world and not use_custom_map:
                    self.world_cache_manager.load_cached_entities()
                else:
                    print("Skipping cached entity loading for new/custom world")
