...
class NPC(Rectangle):
    ...
    def apply_ai_decision(self, decision):
        ...
        # Check if this is advice to follow
        if hasattr(decision, 'following_advice') and decision.following_advice:
            # Store advice attributes
            self.advice_direction = decision.advice_direction
            self.advice_remaining_distance = getattr(decision, 'advice_remaining_distance', 0)
            if self.advice_remaining_distance == 0 and hasattr(decision, 'advice_distance'):
                self.advice_remaining_distance = decision.advice_distance
        ...
        if decision.action == "move_left":
            ...
            if world_map and world_map.is_adjacent_to_water(self.grid_x, self.grid_y):
                ...
                # Record this water discovery in the world cache
                if game_engine and hasattr(game_engine, 'world_cache'):
                    # Find the actual water tile
                    water_x, water_y = None, None
                    for dx, dy in [(0, 0), (0, 1), (1, 0), (0, -1), (-1, 0)]:
                        nx, ny = self.grid_x + dx, self.grid_y + dy
